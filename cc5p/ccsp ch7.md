...
-->部署模型 (公有、私有、社區、混雲) 混合會移除或減少環境安全控制的授權和執行
...
ONF  組織規範框架 Organization (容器的框架、進行分類和使用應用)
ANF  應用規範框架 Application (是ONF任意子級、針對特定業務應用達到其目標信任水平)
ASMP 應用安全管理流程
...
>>>DAM數據庫活動監測(主機、網路)、異常請求或行為法出警告來阻止惡意活動、可基於主機代理或網路
...
```
***API是一套規定、標準、協議和工具用於建構web應用或工具的應用
...
>>>基於標準、依賴xml、高度不容錯、較慢、內置錯誤處理、可工作多協議。
SOAP工作場景
>>>異步處理、有格式規範、有狀態操作
...
```
QoS 服務質量 旨在避免造成過度控制導致性能下降
--僅使用必要和充分安全控制來控制。將風險降低到可接受水準

軟件安全測試
-->漏洞掃描  檢查已存在已知漏洞掃描，不進行利用
-->EULA 最終用戶許可協議中不允許執行活動，若跨出就承擔巨大風險、皆需授權和許可
-->白盒測試 能讀懂代碼+理解安全的能力，需要另組沒有利益衝突測試團隊
-->黑盒測試  不審查源代碼，測試團隊在應用運行的輸入和輸出結果做測試
```
|SAST靜|空指針解引用|線程、質量|無用代碼|不安全加密|複雜注入|
|:--|:--|:--|:--|:--|:--|
|DAST動|配置、補丁級別|運行級別、身分驗證|協議解析|會話管理|第三方、惡意|
|S+DAST|SQLi,XSS|HTTP響應拆分|LDAP,Xpath注入|路徑歷遍|緩衝區溢出|
```
>>>SAST靜態應用安全測試 已知安全缺陷和漏洞再投入正式區前找到、
-->>>比如XSS、SQLi、緩衝溢出、未處理錯誤條件和後門程序比動態應用安全測試更多結果和準確性高．
>>>DAST不需要提供代碼、在運行時找出問題和漏洞的黑盒測試、對HTTP、HTML web接口進行測試

##NIST FICIC用於公用事業和國家級項目的三個主要組件
-->框架核心組件
>>>識別、保護、檢測、響應、恢復
-->配置文件   活動和業務需求一致
-->框架實施層 根據情況確定所處的位置層級 

已核准API 允許其他應用使用web服務、與第三方工具自動化關聯和集成能力、擴展應用的功能、分別進行分析
-->使用API、開發人員依賴API內建的安全控制及所進行的測試和驗證完整性和安全性
-->API有可能沒有進行過驗證或安全測試、使用未經任何驗證API可能導致數據洩漏、身份驗證薄弱、授權應用錯誤
-->操作一個不安全API最糟導致數據外洩、應用更新或更改API可能暴露之前為暴露的內容項
>>>建立合理流程來持續測試和審查API

##軟件供應鏈管理(API方面)
-->開發人員利用第三方API再加上自己代碼開發成API提供給另個應用使用的循環鏈
>>>因為幾乎無法知道數據是否會向其他應用發送

##開源軟件安全  比私有軟件更靈活、也提供基本相同保護
>>>私有軟體可將導致洩漏和其他影響責任推給軟體供應商、供應商對更新和保護專有應用的積極和持續努力
>>>開源通常包括價格低廉有機會自己來審查測試代碼的安全漏洞、開源還允許用戶進行修改、添加或增強功能

-->敏捷 加速開發能更即時交付，而不是應用的壽命和安全要素。

RASP運行應用自我保護 runtime application self protection
-->無人為干預情狀，重心對自身進行配置輔助防止被攻擊。運行時保護因為應用在內存執行、通過自我調用而發揮的作用

##代碼安全審查 
>>>識別和減少應用代碼潛在漏洞，確保具有適當控制措施，透過宣導、安全編碼課程教育
