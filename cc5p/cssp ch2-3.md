...
>>>組織內部通常是DBA是數據託管者，
***託管者通常是雲服務提供商按國際慣例也稱數據處理者data processor
...
--確保稽核員不向管理單位匯報、避免稽核利益衝突使稽核更有效和實用

##雲日誌可由應用程序、操作系統和設備生成 by IDS SIEM
>>>日誌審查和分析通常不是優先工作 通常由安全部門執行的額外責任
>>>日誌審查是索然無味且不斷重覆   特殊員工審查大量數據塞選與正常情況細微差異
>>>日誌審查同時需要新手和富有經驗的人員  新手審查可不必成班過多成本、經驗足夠執行高價值審查
>>>審查員需要對營運具有一定理解  能區分什麼權限的活動和未授權活動
->>避免審查次數過少可能導致分析者忘記常識和分析技巧

->>需考量特定審計需求在供應商合同包含此類說明，設置或去和儲存、決定審計頻率、誰完成、處理過程

數據銷毀和廢棄
--介質和硬件的物理銷毀技術 physical destruction 燒毀、融化、衝擊、切碎，數據脫敏首選
>>>消磁 degaussing 施加超強磁力使介質脫磁、但不適合SSD
>>>複寫 overwriting 隨機字符多次寫入儲存區最後填充全0或1，對大型儲存極耗時
>>>加密擦除 crypto shredding 生成不同密鑰加密數據後毀掉密鑰**雲唯一數據廢棄策略
->>刪除只是處理移除數據的邏輯指針

數據廢棄策略
-->數據廢棄程序、適用合規、銷毀過程清晰指南
-->數據殘留data remanence是指各種數據脫敏和廢棄遺留的任何數據、加密擦除
-->
