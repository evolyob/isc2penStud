...
-蜜罐/密網  外表行為都向合法系統但是不具實際價值數據，不補釘故意留下漏洞。能拖延入侵者足夠長時間盡可能收集入侵者相關信息
...
>基於IP+port帶協議編號的過濾通信流來監控所有進出流量、ACL允許特定通過+以隱式拒絕規則結束(沒有就不給過)、ping使用ICMP在邊界阻止ICMP回升請求
>第二代防火牆增加過濾功能(特定應用程序要求過濾、電路層網關防火牆根據電路過濾)、第三代(狀態、動態包過濾)防火牆根據流量狀態過濾
>
-沙箱
-第三方安全服務
```
