...
-強制訪問控制 MAC 如果具絕密標籤那就能訪問絕密，主客體標籤可以匹配記錄在*格子的模型*
...
-基於規則訪問控制rule-BAC  模型中規則(FWpolicy)也稱為限制或過濾器
**>決定系統允許和禁止發生的操作、主客體訪問權限獲授予主體操作的權限、適用所有主體全局規劃
--->rule-BAC的一个高级实现是基于属性的ABAC
...
||格式|基於||
|:--|:--|:--|:--|
|SAML|XML|認證A授權A|配置用戶資源|
|SAML2.0|XML|AML1.1、OASIS、ID-FF1.2+shibboleth1.3|提供主體認證、授權和屬性信息|
|IdP|XML||認證聲明、授權聲明、屬性聲明|
|OAuth2.0||RFC6749|應用程序從身份提供者獲取訪問令牌|
|OpenID||OASIS維護|一組憑證登入多個不相關網站|
|OIDC|JWT|OAuth2.0|使用JSON Web(ID)令牌發生在後台|
```
互聯網上實現SSO
-多站點支持SSO簡化用戶體驗確保某站點身份憑證不會被其他站點共享、絕不把自己的憑證提供給第三方
-XML可擴展標記語言 XML可包含標記、從描敘所有所需數據、多供應商依XML格式導入導出、成交換信息通用語言、
-->雲提供商XML共享信息身份認證和授權信息

-SAML安全斷言標記語言 '基於XML'開放標準常用於聯邦組織之間交換身份認證和授權AA信息、支持瀏覽器訪問SSO功能
...
OAuth2.0開放授權 'RFC6749' 互聯網工程任務組IETF維護的授權框架、與第三方網站共享帳戶信息、多在線網站支持
...
OpenID(OASIS維護) 開放標準非RFC標準、一組憑證登入多個不相關網站、中心化身份認證標誌互相建立安全通道並要求口令

OIDC(OpenID Connent)  使用OAuth2.0授權框架身份認證層、同時提供身份認證跟授權
*-JWT 使用javaScript對象表示法JSON Web令牌、可稱ID令牌、
--->OIDC使用Web服務檢索JWT的身份認證和有關用戶個人資料、發生在後台
...
Kerberos 票證身分認證AES(不明文傳輸) 證明自己身分簽發票證，訪問資源出示這些票證即可、
--->儲存在目錄服務、NTP校時與主機和所有客戶端保持同步
...
>>登入流程: 輸入帳密>>使用AES加密傳輸到KDC>> 生成一對密鑰,KDC使用用戶口令哈希值隊密鑰加密跟附帶TGT的時戳
--->>回傳客戶端使用值過期
>>使用用戶口令哈希值來解密對稱密鑰
客戶端獲得票證流程
...
RADIUS 遠程認證撥入用戶服務集中訪問連接、VPN，適用多台網路訪問服務器、
-->服務器將用戶憑證轉發至RADIUS*"做身分認證、授權、跟蹤計費(AAA)
...
-彩虹表攻擊  使用大量預計計算的哈希數據庫來減少所需時間、
--->猜測口令的哈希值跟口令放到彩虹表在跟被盜的口令數據庫的哈希值比對直到匹配
--->通常使用加鹽口令減少彩虹表攻擊的有效性、鹽是和哈希口令儲存在同一數據庫的隨機數、
--->所以獲取數據庫後也會得到口令的鹽、胡椒是儲存在別處的很大常數

-mimikatz  讀取內存憑證進行漏洞利用
>從系統服務進程中內存讀取口令和pin、口令哈希值
：：cryptoAPI模塊提取系統證書和相關連的私鑰
：：讀取windows的LM,NTLM哈希值保存在本地安全帳戶管理器SMA的數據或創建哈希值時儲存在內存．

>本地安全機構子系統服務LSASS通常不會以明文出存但惡意修改註冊表、啟用摘要認證就可以讀取口令
>枚舉正在執行進程、使用powershell腳本invoke-mimikatz加載到內存中

-哈希傳遞PtH攻擊 客戶端捕獲生成口令哈希值發送身份驗證服務、攻擊者不需要知道實際口令
：：主要對NTLM(NT LANmanager),Kerberos、其他系統也是很容易
>攻擊流程：利用工具捕獲(lsass.exe)用戶或管理權限最近登入口令哈希值、偽裝用戶,本地系統,遠程進行身份認證、
--> 一但成功就可使用整個網絡
...
>Kerberos暴力破解 因Kerberos會回復用戶是否有效
>ASREPRoast 識別未啟用Kerberos預身分認證用戶，因預身分認證是抵禦口令猜測
>>>攻擊者向KDC發送身分請求>>KDC回復一個票證授予票證TGT>>使用客戶端口令加密。攻擊者可以發起離線攻擊來解密票證並發現客戶端口令
>Kerberoasting 收集加密的票證授予服務TGS票證，TGS由用戶上下文運行服務使用，試圖找有沒有預身分認證的用戶

-嗅探攻擊  捕獲流量數據包分析
>加密通過網路發送所有敏感數據無法輕鬆讀取、使用HTTPS、SFTP、SSH:: 一次性口令可抵禦嗅探
>控制對路由器和交換機的物理訪問、網路監控IDS嗅探器特徵

-欺騙攻擊 偽裝合法登入窗口相同介面去捕獲用戶憑證、
>將有效IP替換虛假IP隱藏或冒充受信任的系統、電子郵件欺騙釣魚、電話號碼VoIP欺騙讓呼叫者號碼替換成外一個或相同區號

```
```
核心保護方法
-對系統物理訪問  一但有口令文件、便可離線破解、成功下載口令文件視為洩漏
-對文件電子訪問  嚴密控制監控重要數據電子訪問包含口令文件、應立即調查未授權的口令數據庫文件訪問行為
-哈希或加鹽口令  額外增加胡椒保護口令、強口令加鹽加胡椒的口令很難被彩虹表或其他攻擊破解
-使用口力掩碼  不明碼顯示降低肩窺、但可透過鍵盤觀察前另輸入

-部署多因素身份認證  做為額外保護措施
-使用帳戶鎖定    住於抵禦口令在線猜測攻擊、錯誤達到定義次數後鎖定、但防不了被盜口令數據庫進行離線破解
-上次登入通知    成功登入時間日期位置、
-用戶安全意識培訓
