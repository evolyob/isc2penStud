...
1--->主引導紀錄MBR  最早感染形式利用加載過城類型(可啟用介質,大小512字節)、在合法引導扇區加載操作系統前將病毒到內存
2-->>文件感染病毒  可執行文件和腳本、可能只是銷為改動執行程序來植入、通過比對或哈希植
-->>變體"同伴病毒"  利用合法操作系統文件類似名稱來躲避檢查，應避免快捷方式並使用具體文件名

3-->>宏病毒  自動腳本語言(VBA)執行功能、限制不受信的宏沒有用戶明確許可運行的能力
4-->>服務注入  感染系統並逃脫檢測的技術、破壞可信的進程繞過防毒檢測、確認所有軟件是最新安全補丁
...
```
病毒依靠人為行動來啟動，並且需要一個主機系統來複製
蠕蟲則不需要主機系統或用戶執行操作來傳播
```
...
勒索軟件  只生成作者知道的'加密密鑰'、加密任何已安裝驅動器關鍵文件讓任何人無法訪問數據
...
反惡意軟件  各端點部署反惡意軟件、基於特徵檢測潛在已知病毒、定期掃描配置設置、儲存介質
...
->>軟件多樣性   避免單點故障影響可用性
--->>關注依賴單個源代碼、二進制可執行文件或編譯器位置、跟蹤他們是維護關鍵任務之一
->>代碼倉庫  儲存管理集中位置、以便安全儲存和協調多人開發人員作出的更改、還執行版本控制需要時可滾回早期版本
--->>開發過程自動審計和變更日誌紀錄、向組織內部公開代碼促進代碼重用而不是從零開始的私有倉庫、避免死代碼：正在使用但是沒人維護

->>完整性度量 通過批准管理流程以安全方式提供和撤銷應用程序
--->>完整性度量使用加密哈希函數驗證使否相同、任何哈希偏差都代表被修改過．
->>應用程序韌性
-->>可擴展性：可以逐漸增加支持不斷增長的需求、水平擴展或向外拓展
-->>彈性：必要時自動提供資源進行拓展、不再需要時自動撤銷降低容量、根據需求上下伸縮的能力
...
內存管理
-->>資源耗盡  避免破壞整個系統的安全性、造成禁用或無法用於其他用途
--->>應用程序沒有返回不需要的內存、可能只是無法跟蹤已寫入保留區域的對象耗盡可用內存導致崩潰、若沒有修正就會開始循環

-->>指針解引用  儲存內存中另一位置地址的內存區域、應用程序會隨著指針並訪問指針地址引用的內存
-->>空指針異常：當指針為空值null、如果應用程式取消引用null指針會導致程序崩潰
-->>攻擊者能夠訪問調試信息用於偵查安全性或允許繞過安全控制
```
