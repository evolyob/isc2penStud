...
>'標準化'封裝、多路復用、連路配置、質量測試、功能協商、自動連接、全雙工、身份認證
...
遠程連接安全
-嚴格身分認證才可獲得訪問權限、工作需要特定人員可建立遠端連線、建立**前需加密技術保護身份和傳輸數據來保護和監控、
...
|基於關聯位置|依據前次連結分配目的地|將後續分配到相同目的地|
```
...
-DKIM   域名關鍵字標示郵件  捅過驗證域名標示來自組織的有效
...
-VLAN邏輯分段不必更改物理網路拓樸、雲分布式虛擬機有助於減少引入配置錯誤和集中管理並可以使用基礎設施及代碼體IaC系結構管理
...
-NAT動態將數據抱頭中內部系統的私有IPv4地址轉換為外部、
-NAT相關的是端口地址轉換(PAT、超載NAT)、網路與端口地址轉換(NPAT)、網路地址與端口轉換(NAPT)允許單公共IPv4來自65,536(限4,000)的內部客戶端同時通信
-SNAT=狀態NAT 原網路地址轉址 映射和IPv4地址套接自分配在會話啟動創建、中斷就解散

-NAT-T(RFC3947)NAT遍歷 因IPsec要求NAT數據包頭更改、傳統不支援IPsecVPN、但支援專用L2TP的隧道模式可跨同邊界設備接口的NAT和VPN獲益
...
WAN(廣域網) 需正確連接管理及傳輸加密、專用線路(dedicated line租用 特定點對點保持等待狀態)常用於多點業務辦公、但基於光纖方案所取代
