...
分布式系統 distributed system/分布式計算環境 DCE ||
-同質成員、共同功能目標操作(對等式架構)
-固有安全問題:組件之間關聯性 
...
```
```
微服務 通常是單向操作很少依賴其他元素 開發週期短，不可變動架構 確保與API之間輸出入 api gateway 身份驗證、授權、流量管制
...
密鑰管理 失敗大歸於密鑰管理非算法，因要無線訪問更強的隨機數生成器RNG，應減少丟失或洩漏風險的儲存方式，
--->最佳選是可移動硬件(microSD HSM)或可信平台模塊TPM
憑證管理  安全儲存大量憑證集的方法，主憑證集來解鎖數劇集，應為程序或網站自動登入選項
-口令庫(passwd vault)憑證軟件管理器，本身是加密必須解鎖才能獲得訪問儲存項目權限．
--->通常用於口令密鑰派生函數(PBKDF2)or bcrypt把主口令換成合理強度的加密密鑰
```
```
自帶設備 BYOD bring your own device 非所有移動設備具備充分安全性能 故不允許進入生產網路．也會把員工自己的個資暴露給組織．
--->公司擁有個人使用 COPE 組織買設備給員工在允許網絡上使用，避免錯誤暴露組織數據的風險或透過設備把惡意軟件帶入，
>員工自身個資被機構訪問． 傳播惡意軟件、業務數據和個人數據混雜在同一設備上可能性
--->公司擁有移動設備戰略COMS corporate-owned mobile strategy
--->公司擁有僅供業務COBO corporate-owned bussiness only
>公司購買可支持安全策略合規設備，只用於公司工作目的明確分開

數據擁有權  按類型隔離數據 應明定數據擁有權力策略，MDM/UEM有不在不影響個人數據分隔功能並支持業務數據清理、
--->備份解決方案降低遠程擦除及設備故障損壞造成數據丟失、明定誰負責設備維修、更換、或技術支持或由個人及服務供應商負責
...
隱私  員工須同意組織對跟蹤和監控，把BYOD CTOD當作準公司資產，簽字同意
退役  正是擦除業務數據、刪除所有業務專用應用程序

可接受使用政策(AUP) 上班是本職完成的任務
-->相機攝像頭、麥克風、共享熱點、
>使用時機 利用MDM/UEM的硬件禁用，進入場域前關閉
```
```
進程隔離   每個進程指令和數據提供在單獨內存空間，阻止其他進程讀寫數據
硬件隔離   阻止人員不同安全擊別訪問
安全策略   日常安全運行、流程、規章、，管理保護分發敏感信息、開發設計建構執行測試安全

隱蔽通道 covert channel 違反、繞道、避開安全策略、不受監控控制
隱蔽時序通道  covert timeing channel 改變時序傳送訊息，秘密傳輸數據方法
隱蔽儲存通道  covert storage channel 側道、放在看不到或忽略的位子，未授權主體交換信息，
-->行為監控異常行為，通過起方式或模式匹配修發已知惡意活動。
rootkit 嵌入操作系統是生根概念取得系統整體或全面控制，更換驅動程式、滲透應用程序庫，讓行為是正常的。防止被工具檢查
-初期可能覺得有點延遲、無響應，惡意軟件是初始感染程序。刪除程序、安裝會執行特權提升(設備驅動程式driver、動態連結庫dll)

增量攻擊
-數據欺騙      獲得訪問權限 進行儲存輸入輸出交易過程對數據微小隨機增輛更改。算hash檢驗
-薩拉米香腸攻擊 帳戶或其他具有財務紀錄資產，慢慢扣除不起眼的金額，通過內部人員參與。職權分離代碼控制


機器類型 :真實、虛擬、多任務、多程序、多處理、多處理器、多用戶
操作模式 : 用戶模式、管理程序、特權模式
儲存模式 : 主要、輔助、真實、虛擬、易失姓(ram)、非易失性(rom)、隨機、順序
